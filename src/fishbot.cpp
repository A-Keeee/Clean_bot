...
SendData_t received_data;        // 存储接收到的传感器数据
...
bool create_fishbot_transport()
{
    ...
    Serial.println("[INFO] Odometry message structure initialized");
    // ========== 消息初始化完成 ==========
    ...
    // ========== 重要：正确初始化字符串字段 ==========
    // 使用 micro_ros_string_utilities_init 而不是 set（因为 memset 后指针为 NULL）
    odom_msg.header.frame_id = micro_ros_string_utilities_init(odom_frameid_str.c_str());
    odom_msg.child_frame_id = micro_ros_string_utilities_init(odom_child_frameid_str.c_str());
    imu_msg.header.frame_id = micro_ros_string_utilities_init("imu");
    
    Serial.printf("[INFO] Frame IDs set: odom='%s', child='%s', imu='imu'\n", 
    ...
    Serial.println("[INFO] Creating odom publisher with RELIABLE QoS (testing)...");
    rcl_ret_t ret_odom_pub = rclc_publisher_init_default(  // 改用 default (reliable) QoS
        &odom_publisher,
        &node,
        ROSIDL_GET_MSG_TYPE_SUPPORT(nav_msgs, msg, Odometry),
        odom_topic.c_str());
    
    if (ret_odom_pub != RCL_RET_OK) {
        Serial.printf("[CRITICAL] Odom publisher init FAILED! Error code: %d\n", (int)ret_odom_pub);
        return false;
    } else {
        Serial.println("[INFO] Odom publisher created successfully with RELIABLE QoS");
    }
    
    Serial.println("[INFO] Creating IMU publisher...");
    rcl_ret_t ret_imu_pub = rclc_publisher_init_best_effort(
        &imu_publisher,
        &node,
        ROSIDL_GET_MSG_TYPE_SUPPORT(sensor_msgs, msg, Imu),
        "imu");
    
    if (ret_imu_pub != RCL_RET_OK) {
        Serial.printf("[CRITICAL] IMU publisher init FAILED! Error code: %d\n", (int)ret_imu_pub);
        return false;
    } else {
        Serial.println("[INFO] IMU publisher created successfully");
    }
    ...
    RCSOFTCHECK(rclc_executor_add_subscription(&executor, &sweep_subscriber, &sweep_msg, &callback_sweep_subscription_, ON_NEW_DATA));
...
bool destory_fishbot_transport()
{
    ...
    RCSOFTCHECK(rcl_subscription_fini(&sweep_subscriber, &node));
...
{   
    ...
    // Serial.println("loop_fishbot_control()");
    if (receive_sensor_data(&received_data)) {
        // Serial.println("Sensor data received:");
        // fishlog_debug("sensor", "Acc: %.2f, %.2f, %.2f | Gyro: %.2f, %.2f, %.2f | Enc_L: %d, Enc_R: %d",
        //               received_data.acc[0], received_data.acc[1], received_data.acc[2],
        //               received_data.gyro[0], received_data.gyro[1], received_data.gyro[2],
        //               received_data.enc_L, received_data.enc_R);
    }
    ...
    kinematics.update_motor_ticks(micros(), -received_data.enc_L, received_data.enc_R);
    ...
    */
    ...
    // 2. 创建并填充测试数据
    MockTwist test_msg;
    test_msg.linear.x = 0.5;   // 模拟线速度 0.5 m/s
    test_msg.linear.y = 0.0;
    test_msg.linear.z = 0.0;
    test_msg.angular.x = 0.0;
    test_msg.angular.y = 0.0;
    test_msg.angular.z = 1.2;   // 模拟角速度 1.2 rad/s

    // 3. 按照指定的帧格式通过Serial2发送数据
    
    // 通过Serial2发送Twist消息数据，0x5A为帧头，0xAA为帧尾
    Serial2.write(0x5A); // 帧头
    
    // 发送线速度(linear.x, linear.y, linear.z) - 3 * 8 = 24 字节
    Serial2.write((uint8_t*)&test_msg.linear.x, sizeof(test_msg.linear.x));
    Serial2.write((uint8_t*)&test_msg.linear.y, sizeof(test_msg.linear.y));
    Serial2.write((uint8_t*)&test_msg.linear.z, sizeof(test_msg.linear.z));
    
    // 发送角速度(angular.x, angular.y, angular.z) - 3 * 8 = 24 字节
    Serial2.write((uint8_t*)&test_msg.angular.x, sizeof(test_msg.angular.x));
    Serial2.write((uint8_t*)&test_msg.angular.y, sizeof(test_msg.angular.y));
    Serial2.write((uint8_t*)&test_msg.angular.z, sizeof(test_msg.angular.z));
    
    Serial2.write(0xAA); // 帧尾
    */
...
void callback_sensor_publisher_timer_(rcl_timer_t *timer, int64_t last_call_time)
{
    ...
    if (timer != NULL)
    {
        // 检查是否已连接到agent
        if (state != AGENT_CONNECTED) {
            Serial.println("[WARN] Timer fired but not connected to agent, skipping publish");
            return;
        }
        
        // 检查 publisher 是否已初始化
        if (!rcl_publisher_is_valid(&odom_publisher)) {
            Serial.println("[ERROR] Odom publisher not initialized!");
            return;
        }
        ...
        // ========== 调试模式：发布固定的 odom 数据 ==========
        // 取消下面的注释以启用固定数据调试模式
        // #define DEBUG_FIXED_ODOM 1
        
        #ifdef DEBUG_FIXED_ODOM
        // 发布固定的测试数据，用于验证 micro-ROS 连接是否正常
        static float debug_x = 0.0;
        static float debug_y = 0.0;
        static float debug_yaw = 0.0;
        static uint32_t debug_counter = 0;
        
        // 每次调用增加一点位置，模拟机器人移动
        debug_x += 0.001;  // 每次增加 1mm
        debug_y += 0.0005; // 每次增加 0.5mm
        debug_yaw += 0.01; // 每次增加约 0.57 度
        debug_counter++;
        
        // 限制 yaw 在 -PI 到 PI 范围内
        while (debug_yaw > PI) debug_yaw -= 2 * PI;
        while (debug_yaw < -PI) debug_yaw += 2 * PI;
        
        // 计算四元数 (简化版，只考虑 z 轴旋转)
        float half_yaw = debug_yaw * 0.5;
        float qw = cos(half_yaw);
        float qz = sin(half_yaw);
        ...
        odom_msg.pose.pose.position.x = debug_x;
        odom_msg.pose.pose.position.y = debug_y;
        ...
        // 固定的速度值
        odom_msg.twist.twist.linear.x = 0.1;   // 0.1 m/s
        odom_msg.twist.twist.linear.y = 0.0;
        odom_msg.twist.twist.linear.z = 0.0;
        
        odom_msg.twist.twist.angular.x = 0.0;
        odom_msg.twist.twist.angular.y = 0.0;
        odom_msg.twist.twist.angular.z = 0.2;  // 0.2 rad/s
        
        // 注意：协方差矩阵已在 create_fishbot_transport() 中初始化
        // 这里可以选择性地设置对角线元素为实际方差值
        ...
        float debug_linear = 0.1;
        float debug_angular = 0.2;
        display.updateBotAngular(debug_angular);
        display.updateBotLinear(debug_linear);
        
        Serial.printf("[DEBUG] Publishing fixed odom #%d: x=%.3f, y=%.3f, yaw=%.3f rad\n", 
                     debug_counter, debug_x, debug_y, debug_yaw);
        #else
        // 正常模式：从运动学模型获取实际数据
        ...
        // 在发布前检查消息完整性
        // Serial.printf("[DIAG] Before publish - frame_id: %s, child_frame_id: %s\n", 
        //              odom_msg.header.frame_id.data, 
        //              odom_msg.child_frame_id.data);
        // Serial.printf("[DIAG] Publisher valid: %d\n", rcl_publisher_is_valid(&odom_publisher));
        ...
        // 如果发布失败，打印详细错误信息
        if (ret != RCL_RET_OK) {
            Serial.printf("[ERROR] Odom publish failed with code: %d\n", (int)ret);
            Serial.printf("[ERROR] RCL error codes: OK=%d, ERROR=%d, TIMEOUT=%d\n", 
                         RCL_RET_OK, RCL_RET_ERROR, RCL_RET_TIMEOUT);
            
            // 检查消息字段是否被破坏
            Serial.printf("[DIAG] After failed publish - frame_id ptr: %p, data: %s\n", 
                         odom_msg.header.frame_id.data,
                         odom_msg.header.frame_id.data ? odom_msg.header.frame_id.data : "NULL");
        ...
        // {
            ...
            imu_msg.angular_velocity.x = received_data.gyro[0];
            imu_msg.angular_velocity.y = received_data.gyro[1];
            imu_msg.angular_velocity.z = received_data.gyro[2];

            imu_msg.linear_acceleration.x = received_data.acc[0];
            imu_msg.linear_acceleration.y = received_data.acc[1];
            imu_msg.linear_acceleration.z = received_data.acc[2];
            
            // 从欧拉角转换为四元数
            quaternion_t quat;
            kinematics.Euler2Quaternion(received_data.angle[0],  // Roll
                                        received_data.angle[1],  // Pitch
                                        received_data.angle[2],  // Yaw
                                        quat);
...
void callback_twist_subscription_(const void *msgin)
{
    ...
    //ROS2 Twist消息中的线速度单位为m/s，角速度单位为rad/s
    //小鱼机器人的运动学模型中，电机转速单位为mm/s
    ...
    // 通过Serial2发送Twist消息数据，0x5A为帧头，0xAA为帧尾
    Serial2.write(0x5A); // 帧头
    // 发送线速度(linear.x, linear.y, linear.z)
    Serial2.write((uint8_t*)&msg->linear.x, sizeof(msg->linear.x));
    Serial2.write((uint8_t*)&msg->linear.y, sizeof(msg->linear.y));
    Serial2.write((uint8_t*)&msg->linear.z, sizeof(msg->linear.z));
    // 发送角速度(angular.x, angular.y, angular.z)
    Serial2.write((uint8_t*)&msg->angular.x, sizeof(msg->angular.x));
    Serial2.write((uint8_t*)&msg->angular.y, sizeof(msg->angular.y));
    Serial2.write((uint8_t*)&msg->angular.z, sizeof(msg->angular.z));
    // 发送sweep控制指令(后4位分别控制4个电机)
    Serial2.write(sweep_msg.data);
    Serial2.write(0xAA); // 帧尾
}

// 用于处理接收到的Sweep类型的ROS消息
void callback_sweep_subscription_(const void *msgin)
{
    const std_msgs__msg__UInt8 *msg = (const std_msgs__msg__UInt8 *)msgin;
    // 保存sweep值(后4位分别控制4个电机: bit0-motor1, bit1-motor2, bit2-motor3, bit3-motor4)
    sweep_msg.data = msg->data & 0x0F;  // 只保留后4位
    // 打印调试信息
    fishlog_debug("sweep", "Received sweep command: 0x%02X (Motor1:%d Motor2:%d Motor3:%d Motor4:%d)", 
                  sweep_msg.data,
                  (sweep_msg.data & 0x01) ? 1 : 0,  // bit0 - 电机1
                  (sweep_msg.data & 0x02) ? 1 : 0,  // bit1 - 电机2
                  (sweep_msg.data & 0x04) ? 1 : 0,  // bit2 - 电机3
                  (sweep_msg.data & 0x08) ? 1 : 0); // bit3 - 电机4
}
...
// 解包传感器电机数据
bool Unpack_SensorMotorData(const uint8_t *buf, uint16_t len, SendData_t *data) {
    if (len != 57 || buf[0] != 0x5A || buf[56] != 0xAA) {
        Serial.println("========== Frame Error ==========");
        Serial.printf("Expected len=57, got len=%d\n", len);
        Serial.printf("Expected header=0x5A, got header=0x%02X\n", buf[0]);
        Serial.printf("Expected footer=0xA5, got footer=0x%02X\n", buf[56]);
        Serial.print("Raw buffer (first 10 bytes): ");
        for (int i = 0; i < min(10, (int)len); i++) {
            Serial.printf("0x%02X ", buf[i]);
        }
        Serial.println();
        Serial.print("Raw buffer (last 10 bytes): ");
        for (int i = max(0, (int)len - 10); i < len; i++) {
            Serial.printf("0x%02X ", buf[i]);
        }
        Serial.println();
        Serial.println("=================================");
        return false; // 长度或帧头帧尾错误
    }
    uint16_t idx = 1; // 跳过帧头

    // 加速度
    for (int i = 0; i < 3; i++) {
        if (buf[idx++] != 0x01 + i) return false;
        memcpy(&data->acc[i], &buf[idx], sizeof(float));
        idx += sizeof(float);
    }

    // 角速度
    for (int i = 0; i < 3; i++) {
        if (buf[idx++] != 0x11 + i) return false;
        memcpy(&data->gyro[i], &buf[idx], sizeof(float));
        idx += sizeof(float);
    }

    // 角度
    for (int i = 0; i < 3; i++) {
        if (buf[idx++] != 0x21 + i) return false;
        memcpy(&data->angle[i], &buf[idx], sizeof(float));
        idx += sizeof(float);
    }

    // 编码器
    if (buf[idx++] != 0x31) return false;
    memcpy(&data->enc_L, &buf[idx], sizeof(int32_t));
    idx += sizeof(int32_t);
    if (buf[idx++] != 0x32) return false;
    memcpy(&data->enc_R, &buf[idx], sizeof(int32_t));
    idx += sizeof(int32_t);

    return true;
}

// 从Serial2接收传感器电机数据 - 只保留最新帧，丢弃旧数据
bool receive_sensor_data(SendData_t *data) {
    static uint8_t buffer[57];
    static uint16_t buffer_idx = 0;
    static bool in_frame = false;
    
    // 如果缓冲区数据过多(超过2帧)，清空旧数据，只保留最新的
    int available_bytes = Serial2.available();
    if (available_bytes > 114) {  // 2帧的数据量
        // 计算需要丢弃的字节数，保留最后约1.5帧的数据
        int bytes_to_skip = available_bytes - 85;
        while (bytes_to_skip > 0 && Serial2.available()) {
            Serial2.read();
            bytes_to_skip--;
        }
        // 重置状态，重新寻找帧头
